...
```{r}
sj_tree <- subset(sj_tree, select=-c(nseries))
caH_tree <- subset(caH_tree, select=-c(nseries))
caL_tree <- subset(caL_tree, select=-c(nseries))
...
s %>% write.csv(file = here::here('/out/bai_resiliences_severes' ,'resiliences_bai_droughts.csv'), row.names = FALSE) 
```
...
```{r}
# Variante todos juntos 
resi_temporal_one <- s %>% filter(samps >= 10) %>% 
  ggplot(aes(y=rs_mean, x=disturb_year, colour=site)) + 
  geom_errorbar(aes(ymin=rs_mean - rs_sd, 
                    ymax=rs_mean + rs_sd),
                width = 0.75, 
                size = .7,
                position=position_dodge(2)) +
  geom_point(aes(size=d_severity), shape=21, fill='white', position=position_dodge(2)) + 
...
pdf(here::here('/out/bai_resiliences_severes', 'res_bai_longterm_one.pdf'), width=12, height = 9)
resi_temporal_one
dev.off()
```
...
plotrel <- function(df, v, x, label, XLab, YLab, coordX_eq, coordY_eq){
  require(ggplot2)
  require(ggrepel)
...
# relation con drought intensity 
```{r}

intensityRs <- plotrel(sdf, v = "rs", x = 'd_intensity', label = 'disturb_year',
        XLab = "Drought Intensity", YLab = "Resilience (Rs) (3year window size)",
        coordX_eq = -1.5, coordY_eq = 0.5)

intensityRt <- plotrel(sdf, v = "rt", x = 'd_intensity', label = 'disturb_year',
        XLab = "Drought Intensity", YLab = "Resistance (Rt) (3year window size)",
        coordX_eq = -1.5, coordY_eq = 0.5) 

intensityRc <- plotrel(sdf, v = "rc", x = 'd_intensity', label = 'disturb_year',
        XLab = "Drought Intensity", YLab = "Recovery (Rc) (3year window size)",
        coordX_eq = -1.5, coordY_eq = 0.5) + ylim(c(0.5,3.5)) 

pdf(here::here('/out/bai_resiliences_severes', 'rs_vs_drought_3years_intensity.pdf'), width=18, height = 7)
intensityRt + intensityRc + intensityRs 
dev.off()
```
...
```{r sin_95}
sdf95 <- sdf %>% filter(disturb_year != 1995)

sevRt95 <- plotrel(sdf95, v = "rt", x = 'd_severity', label = 'disturb_year',
                 coordX_eq = 6, coordY_eq = 0.5, titulo = "Rt") +
  xlab('') + ylab('') +
  theme(plot.margin = margin(r=-0.7, l=0, t=0, b=0, "pt"))


sevRs95 <- plotrel(sdf95, v = "rs", x = 'd_severity', label = 'disturb_year',
                 coordX_eq = 6, coordY_eq = 0.5, titulo = "Rs") + 
  xlab('') + ylab('') +
  theme(plot.margin = margin(r=0, l=-0.7, t=0, b=0, "pt"))

sevRc95 <- plotrel(sdf95, v = "rc", x = 'd_severity', label = 'disturb_year',
                 coordX_eq = 6, coordY_eq = 0.5, titulo = "Rc") +
  xlab("Drought Severity") + ylab('') +
  theme(plot.margin = margin(r=-0.7, l=-0.7, t=0, b=0, "pt"))

res_long_term95 <- sevRt95 + sevRc95 + sevRs95 + 
  plot_layout(nrow = 1) 

res_long_term95


pdf(here::here('/out/bai_resiliences_severes', 'rs_vs_drought_3years_95.pdf'), width=18, height = 7)
res_long_term95
dev.off()

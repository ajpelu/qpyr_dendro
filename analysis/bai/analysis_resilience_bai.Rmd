---
title: "Growth Resilience (reanalisis)"
...
# Compute diameter (mm)
compete <- compete %>% 
  dplyr::mutate(dn_mm = (perim_mm / base::pi))
...
write.csv(aux_coefs, file=here::here('/out/correla_resilience', 'correla_window_size.csv'), row.names = F)
```
...
```{r growth_disturbances_boxplot, echo = FALSE}
...
# Export csv 
write.csv(g, file=here::here('/data/resilience', 'crecimientos_drought.csv'), row.names = FALSE)
...
## Mean + se 
```{r growth_disturbances_mean_se, echo = FALSE}
g %>% 
...
```{r growth_disturbances_mean_se_colours, echo = FALSE}
pgrowht <- g %>% 
  mutate(disturb = dplyr::recode(disturb, dr = '1_dr', pre = '0_pre', post = '2_post')) %>%
  group_by(disturb, disturb_year, site) %>% 
  summarise(mean = mean(mean_period),
...
ggsave(plot=pgrowht, width=8, height = 4,
       filename=here::here('/out/mean_resilience/','bai_events.pdf')) 
```
...
# Export csv 
write.csv(re, file=here::here('data/resilience/','resilience_bai.csv'), row.names = FALSE)
```
...
normtestA %>% pander()
write.csv(normtestA, 
          file=here::here('/out/anovas_resilience/bai', 'normo_disturb_year.csv'), row.names = F)
```
...
normtestB %>% pander()
write.csv(normtestB, 
          file=here::here('/out/anovas_resilience/bai','normo_site.csv'), row.names = F)
```
...
write.csv(normtestAB, 
          file=here::here('/out/anovas_resilience/bai', 'normo_disturb_year_site.csv'), row.names = F)
```
...
homo %>% pander()
write.csv(homo, 
          file=here::here('/out/anovas_resilience/bai','homocedasticidad.csv'), row.names = F)
```
...
homo_log %>% pander()
write.csv(homo_log, 
          file=here::here('/out/anovas_resilience/bai/','homocedasticidad_log.csv'), row.names = F)
```
...
```{r aov_rc_site, fig.width = 4, fig.height=4}
ps
```

```{r aov_rc_disturb, fig.width = 4, fig.height=4}
pd
```

```{r aov_rc_inter_collapse, fig.width = 4, fig.height=4}
picollapse
```

```{r aov_rc_inter, fig.width = 6}
pi
```
...
```{r aov_rt_site, fig.width = 4, fig.height=4}
ps
```

```{r aov_rt_disturb, fig.width = 4, fig.height=4}
pd
```

```{r aov_rt_inter_collapse, fig.width = 4, fig.height=4}
picollapse
```

```{r aov_rt_inter, fig.width = 6}
pi
```
...
```{r aov_rrs_site, fig.width = 4, fig.height=4}
ps
```

```{r aov_rrs_disturb, fig.width = 4, fig.height=4}
pd
```

```{r aov_rrs_inter_collapse, fig.width = 4, fig.height=4}
picollapse
```

```{r aov_rrs_inter, fig.width = 6}
pi
```
...
```{r aov_rs_site, fig.width = 4, fig.height=4}
ps
```

```{r aov_rs_disturb, fig.width = 4, fig.height=4}
pd
```

```{r aov_rs_inter_collapse, fig.width = 4, fig.height=4}
picollapse
```

```{r aov_rs_inter, fig.width = 6}
pi
```
...
### mean + sd
```{r plot_resiliences_bai_sd,fig.width = 12, fig.height=12}
grid.arrange(plot_mdSE, plot_msSE, plot_mdsSE, ncol=3)
```

### mean + ci
```{r plot_resiliences_bai_ci, fig.width = 12, fig.height=12}
grid.arrange(plot_mdCI, plot_msCI, plot_mdsCI, ncol=3)
```

```{r, echo=FALSE}
pdf(here::here('/out/mean_resilience/bai','interaction_plotsSE.pdf'), width=9, height = 9)
grid.arrange(plot_mdSE, plot_msSE, plot_mdsSE, ncol=3)
dev.off()

pdf(here::here('/out/mean_resilience/bai','interaction_plotsCI.pdf'), width=9, height = 9)
grid.arrange(plot_mdCI, plot_msCI, plot_mdsCI, ncol=3)
dev.off()
```
...
write.csv(aovas_coeff, file=here::here('/out/anovas_resilience/bai','anovas_statistics.csv'), row.names = F)
...
write.csv(aovas_model_summary, 
          file=here::here('/out/anovas_resilience/bai','anovas_summary_modelos.csv'), row.names = F)
...
mhuber_agg <- mhuber %>% 
  mutate_at(.vars = c("M.Huber", "lower.ci", "upper.ci"), funs(round(., 4))) %>%
  dplyr::select(var, disturb_year:Letter) %>% as.data.frame() 
...
mhuber_agg_b <- mhuber_b %>% 
  mutate_at(.vars = c("M.Huber", "lower.ci", "upper.ci"), funs(round(., 4))) %>% 
  dplyr::select(var, site:Letter) %>% as.data.frame()
...
pdf(here::here('out/resilience_robust/bai','robust_plot_bai_drought.pdf'), width=9, height = 9)
robust_plot_bai_drought 
dev.off()
```

### Response ~ (x=site)
```{r robust_plot_bai_site}
pd <- position_dodge(.2)
...
pdf(here::here('out/resilience_robust/bai','robust_plot_bai_site.pdf'), width=9, height = 9)
robust_plot_bai_site 
dev.off()
```
...
```{r}
# Export data 
write.csv(mhuber, file=here::here('/out/anovas_resilience/huber_bai/','robust_mhuber.csv'), row.names = F)
write.csv(mhuber_agg, file=here::here('/out/anovas_resilience/huber_bai/','robust_mhuber_agg.csv'), row.names = F)
write.csv(mhuber_a, file=here::here('/out/anovas_resilience/huber_bai/','robust_mhuber_a.csv'), row.names = F)
write.csv(mhuber_agg_a, file=here::here('/out/anovas_resilience/huber_bai/','robust_mhuber_agg_a.csv'), row.names = F)
write.csv(mhuber_b, file=here::here('/out/anovas_resilience/huber_bai/','robust_mhuber_b.csv'), row.names = F)
write.csv(mhuber_agg_b, file=here::here('/out/anovas_resilience/huber_bai/','robust_mhuber_agg_b.csv'), row.names = F)

write.csv(pairwise, file=here::here('/out/anovas_resilience/huber_bai/','robust_pairwise.csv'), row.names = F)

write.csv(trimmedanovas, file=here::here('/out/anovas_resilience/huber_bai/','trimmed_anovas.csv'), row.names = F)
```

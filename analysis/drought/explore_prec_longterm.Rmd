...
aux_SJ <- aux %>% filter(code %in% c("5501","5502","5504","5504A","5505","5509","5509A")) %>% mutate(site = "SJ")
aux_CA <- aux %>% filter(code %in% c("6242U","6243E","6248","6257")) %>% mutate(site = "CA")

prec_sites <- aux_SJ %>% bind_rows(aux_CA)

prec_sites_mean <- prec_sites %>% 
  dplyr::select(-code) %>% 
  group_by(hyear, site) %>% 
     dplyr::summarise(
     csum_mean = mean(csum, na.rm = TRUE),
...
prec_sites_mean %>% filter(hyear > 1990) %>% 
  ...
  scale_y_continuous(breaks=seq(0,1500, by=250)) +
  geom_errorbar(aes(ymin = csum_mean - csum_se, ymax= csum_mean + csum_se), width=.1, position = pos) +
  geom_point(size=2, position = pos) + theme_bw() + 
  theme(panel.grid.minor = element_blank()) +
  ylab('Cumulative Precipitation (hydrological year)') + xlab('year') 

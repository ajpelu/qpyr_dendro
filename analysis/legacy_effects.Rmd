...
mlegacy <- legacy %>% group_by(site, ypd) %>% 
  summarise(mean=mean(lag, na.rm=T),
            sd = sd(lag, na.rm=T),
            se = sd/sqrt(length(lag))) 


legacy %>% ggplot(aes(x = ypd, y = lag, colour = site)) + geom_point() + geom_line()

mlegacy %>% ggplot(aes(x = ypd, y = mean, colour = site)) + 
  geom_point() + 
  geom_errorbar(aes(ymin=mean - se, ymax=mean + se)) 


ggplot(aes(x=ypd, y=lag, colour=site, group=site)) + geom_boxplot()
...
# Leer datos de RWI 
* Son datos de las series detrended ([./analysis/chronos/analysis_splines.md]('./analysis/chronos/analysis_splines.md)), 

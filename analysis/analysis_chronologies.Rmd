...
```{r, warning=FALSE, message=FALSE}
# Metodo correlation
tipos_correla <- c('spearman', 'pearson')
...
for (j in 1:40){
  ...
  pearson <- cor.test(aux1$H, aux1$L, method = metodo_correla)$estimate 
  
  ## Bootstratp to ger CI 
  correlationTest <- function(x,index){
    ct <- cor.test(x$H[index], x$L[index], method = metodo_correla)
    return(ct$estimate)
    }
  ...
  # Output results 
  estimate <- bci$t0 # Pearson coefficient
  ci_lower <- bci$bca[4] # Lower CI
  ci_upper <- bci$bca[5] # Upper CI
 
  out <- as.data.frame(cbind(estimate, 
                             ci_lower, 
                             ci_upper))
...
correla$metodo_correla <- metodo_correla
assign(paste0('correla_', metodo_correla), correla)
} 


```

### Plot similarity among chronologies 
...
correla <- rbind(correla_pearson, correla_spearman)
correla$series <- ifelse(correla$site == 'CA', 
                         'CAHigh-CALow',
                         'SJHigh-SJLow')

correla %>% 
  ggplot(aes(x=size, y=estimate, colour=series)) + geom_line()+
  theme_bw() + 
  ...
  geom_line(aes(x=size, y=ci_lower, colour=series), linetype = 2)  +
  geom_line(aes(x=size, y=ci_upper, colour=series), linetype = 2) + 

...
dendro_worldclim <- data.frame(
  ...
  site = fw$site,
  loc = fw$loc, 
  prec = extract(prec_annual, fw),
  tavg = extract(tmean, fw)
)


dendro_worldclim <- dendro_worldclim %>% group_by(site) %>% summarise(p = mean(prec), t = mean(tavg))
...
sur %>% ggplot(aes(y=prec_avg, x=year)) + geom_point(aes(size = n)) + geom_smooth()

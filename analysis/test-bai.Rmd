...
# Name of the tree to be selected  
id_tree <- paste0('SNA',i)

# Name to store the mean value 
name_avg <- paste0('a_',i)
...
# Get year (rownames)
df <- sj %>% mutate(year=rownames(.))

# Get name of the trees 
# Name of each serie: i.e.:  SNA0101 (SNA|id_tree|id_core) 
name_trees <- unique(stringr::str_sub(colnames(sj), start=1, end=5))

# name <- name_trees[1]
name <- name_trees[i]

aver <- df %>% 
  gather(variable, value, starts_with(name)) %>%  
  group_by(year) %>%  
...
for (i in seq_along(name_trees)) {
  name <- name_trees[i]
  
  name_avg <- paste0('a_',stringr::str_sub(name, star=4, end=5))
  
  aux <- sj %>% select(starts_with(name)) %>% rowMeans() %>% as.data.frame()
  names(aux) <- paste0('a_',stringr::str_sub(name, star=4, end=5))
...
name_rwsei
code_site <- 'SNA' 
id_tree <- stringr::str_sub('SNA0102', 4,5)
id_replicate <- 

mm <- sj %>% select(SNA0101, SNA0102) %>% mutate(SNA01 = rowMeans(.)) 



mutate(year=rownames_to_column(.)) %>%

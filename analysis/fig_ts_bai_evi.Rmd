...
```{r}
# Read diameters data
cro <- read.csv(file=paste0(di, '/data/cronos_medias/cronos_sites.csv'), header=TRUE, sep=',')

# evi data from 
# https://github.com/ajpelu/qpyr_modis_resilience/blob/master/data/evi_mean.csv 
...
evi_year <- evi %>% group_by(year, clu_pop2) %>% 
  summarise(mean = mean(evi, na.rm=TRUE),
            sd = sd(evi),
            se = sd /sqrt(length(sd))) %>% as.data.frame()
```
...
cro <- cro %>% 
  dplyr::rename(mean = bai_mean, sd = bai_sd, se = bai_se) %>%
  dplyr::select(-n_trees) %>% 
  mutate(clu_pop2 = as.factor(case_when(
      site =='SJ' ~ 'Northern slope',
      site =='CA_Low' ~ 'Southern slope',
      site =='CA_High' ~ 'Southern slope')),
      variable = 'bai') 
...
psj <- cro %>% 
  filter(year > 1949, site=='SJ') 
  
  ggplot(aes(x=year, y=bai_mean/100)) + 
  theme_bw() + ylab('BAI') + 
  geom_ribbon(aes(ymin = (bai_mean - bai_se)/100,
                  ymax = (bai_mean + bai_se)/100), alpha=.2, colour=NA) +
  geom_line() + 
  geom_point(size=.8) + 
  theme(panel.grid = element_blank(),
        legend.position = c(0.1,.8),
...
psj + geom_line(evi_year, aes(x=year, y=mean*100))
...
p + geom_line(evi_year, aes(x=year, y=mean*100))

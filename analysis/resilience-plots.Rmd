...
```{r}
re_bai <- read.csv(file=paste0(di, 'data/resilience/resilience_bai.csv'), header=TRUE, sep=',')
re_evi <- read.csv(file=paste0(di, 'data/resilience/resilience_evi.csv'), header=TRUE, sep=',')
```


```{r}

bai <- re_bai %>% dplyr::select(-tree, -rrs) %>% mutate(mivariable = "BAI")
evi <- re_evi %>% dplyr::select(-iv_malla_modi_id, -rrs) %>% mutate(mivariable = "EVI")                         
...
* Partimos de los datos obtenidos de resiliencia para los RWI ([./analysis/chronos/analysis_splines.md]('./analysis/chronos/analysis_splines.md)) )
...
re$disturb_year <- as.factor(as.character(re$disturb_year))
...
homo %>% pander()
write.csv(homo, 
          file=paste0(di, '/out/anovas_resilience/rwi/homocedasticidad.csv'), row.names = F)
```
...
homo_log %>% pander()
write.csv(homo_log, 
          file=paste0(di, '/out/anovas_resilience/rwi/homocedasticidad_log.csv'), row.names = F)
```
...
### mean + sd
```{r plot_resiliences_rwi_sd,fig.width = 12, fig.height=12}
grid.arrange(plot_mdSE, plot_msSE, plot_mdsSE, ncol=3)
```

### mean + ci
```{r plot_resiliences_rwi_ci, fig.width = 12, fig.height=12}
grid.arrange(plot_mdCI, plot_msCI, plot_mdsCI, ncol=3)
```

```{r, echo=FALSE}
pdf(paste0(di, '/out/mean_resilience/rwi/interaction_plotsSE.pdf'), width=9, height = 9)
grid.arrange(plot_mdSE, plot_msSE, plot_mdsSE, ncol=3)
dev.off()

pdf(paste0(di, '/out/mean_resilience/rwi/interaction_plotsCI.pdf'), width=9, height = 9)
grid.arrange(plot_mdCI, plot_msCI, plot_mdsCI, ncol=3)
dev.off()
```
...
write.csv(aovas_coeff, file=paste0(di, '/out/anovas_resilience/rwi/anovas_statistics.csv'), row.names = F)

...
pdf(paste0(di, '/out/mean_resilience/evi/interaction_plotsCI.pdf'), width=9, height = 9)
grid.arrange(plot_mdCI, plot_msCI, plot_mdsCI, ncol=3)
dev.off()
```
...
write.csv(aovas_coeff, file=paste0(di, '/out/anovas_resilience/evi/anovas_statistics.csv'), row.names = F)
...
write.csv(aovas_model_summary, 
          file=paste0(di, '/out/anovas_resilience/evi/anovas_summary_modelos.csv'), row.names = F)
...
robustANOVA <- function(df, resp_var, factores, 
  ...
  out_ratr_df <- data.frame(var = c(out_raTrimmed$varnames[2], 
...
write.csv(trimmedanovas, file=paste0(di, '/out/anovas_resilience/huber_evi/trimmed_anovas.csv', row.names = F)
```

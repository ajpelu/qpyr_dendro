...
robustANOVA <- function(df, resp_var, factores, 
  ...
  phab <- pairwiseRobustTest(formula_AB, data = df, est = "mom", 
                            nboot = nboot, method="bonferroni")
  ## letters
  letters_pha <- cldList(comparison = pha$Comparison,
        p.value    = pha$p.adjust,
        threshold  = treshold)
  letters_phb <- cldList(comparison = phb$Comparison,
        p.value    = phb$p.adjust,
        threshold  = treshold)
  
  letters_phab <- cldList(comparison = phab$Comparison,
  ...
  out$letters_pha <- if(exists(letters_pha), letters_pha,  # Letters comparison posthoc
  out$letters_phb <- letters_pha # Letters comparison posthoc
  out$letters_phab <- letters_pha # Letters comparison posthoc

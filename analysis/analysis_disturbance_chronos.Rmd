...
xx <- gc_trees %>% 
  mutate(per_negn = (per_neg)*(-1)) 

gc_trees %>% ggplot() +
  geom_bar(aes(x=year, y=per_pos), fill='blue', stat = "identity", position = "dodge") +
  geom_bar(aes(x=year, y=(-1)*per_neg), fill='blue',  stat = "identity", position = "dodge") + 
...
gc_trees %>% ggplot() + 
  geom_point(aes(x=year, y=per_pos), shape=17) + 
  geom_point(aes(x=year, y=per_neg), shape=6) +
  facet_wrap(~site, nrow = 3) 
...
t_caL <- gc_todos %>% filter(site == 'gc_caL') %>% 
  GC_trees(treshold = 50) %>% data.frame()
t_caL$site <- 'gc_caL'


t_caH <- gc_todos %>% filter(site == 'gc_caH') %>% 
  GC_trees(treshold = 50) %>% data.frame()
t_caH$site <- 'gc_caH'

t_sj <- gc_todos %>% filter(site == 'gc_sj') %>% 
  GC_trees(treshold = 50) %>% data.frame()
t_sj$site <- 'gc_sj'


per_t <- rbind(t_caH, t_caL, t_sj)

per_t %>% ggplot(aes(x=year, y=gc_m)) + 
  geom_bar(stat='identity', fill='gray', colour='gray') + theme_bw() + 
  # geom_point() +
  geom_errorbar(aes(ymin = gc_m - gc_m_se, ymax = gc_m + gc_m_se), colour='gray') +
  facet_wrap(~site, nrow = 3, scales = 'free_y',
...
xx %>% group_by(year, gc_mean) %>% count(tree) %>% filter(gc_mean > 50)



p %>% filter(site=='sj') %>% 
  ggplot(aes(x=year, y=mgc_md)) + 
...
gc_caH %>% 
  group_by(year) %>% 
  summarise(nt = length(tree),
            mgc = mean(gc)) %>% 
  ggplot(aes(x=year, y=mgc)) + geom_bar(stat='identity') + theme_bw() + 
  geom_hline(yintercept=c(25, -25), linetype=2, colour='blue') +
  geom_hline(yintercept=c(50, -50), linetype=2, colour='red')  
...
# + ylim(c(-300,300))
	...
	GCper=100*((mean(m2[,1])-mean(m1[,1]))/mean(m1[,1]))  ##PGC
	
	GCperneg=100*((mean(m1[,1])-mean(m2[,1]))/mean(m2[,1])) ###NGC
...
xx <- getGC(caL_tree, ws=10, prefijo='CA')


GCfin2 <- data.frame(GCper = GCper, GCperneg = GCperneg)
...
AGC2=function(RWL){
  ...
  for (n in 1:ncol(RWL)){
    ...
    if(!is.na(tree2)){
      tree3=as.numeric(substr(colnames(RWL[n]),3,5))}
    else {tree3=tree}
  ...
  GC2$tree2=1
  for (i in 2:nrow(GC2)){
    if(GC2$year[i]<GC2$year[i-1]) GC2$tree2[i]=1+GC2$tree2[i-1] else GC2$tree2[i]=GC2$tree2[i-1]}
  GC2<<-GC2
  }
...
GCfin2$GCfinal=
  ifelse(GCfin2$GCper>0, GCfin2$GCper, -1*GCfin2$GCperneg) ###pongo para los positivos el PGC, para los negativos el NGC
GCfin2<<-GCfin2
}
...
rangej <- (min(series$year)+n):(max(series$year)-n)
j <- rangej[1]

M2=subset(series, series$year>=j & series$year<(j+n-1))
M2b =subset(series, series$year<(j+n-1))
...
range <- (min(rr$year) + 10):(max(rr$year) - 10)

series <- subset(rr, tree2 == 1)
M2 = subset(series, series$year>=j & series$year<(j+n-1))

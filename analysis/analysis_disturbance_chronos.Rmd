...
## Númber of trees 
GC_trees <- function(gcdf, treshold){ 
  ...
  n_pos <- gcdf %>% filter(gc_m > th) %>% group_by(year) %>% summarise(n_pos = length(ntrees)) 
  
  # Get n tree with GC negative lower treshold 
  n_neg <- gcdf %>% filter(gc_m < (-1*th)) %>% group_by(year) %>% summarise(n_neg = length(ntrees)) 
...
treesth <- GC_trees(gcdf = gc_todos, treshold = 50)

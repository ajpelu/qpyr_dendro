...
# Read tree data
tree <- read.csv(file=here::here('/data_raw/', 'dendro_competence.csv'), header=TRUE, sep=',')

# Read spatial data 
# Centroid of focal tree 
field_work_ca <- readOGR(here::here("/data_raw/geoinfo","dendro_ca.GPX" ), layer="waypoints", verbose = FALSE)
field_work_sj <- readOGR(here::here("/data_raw/geoinfo","dendro_sj.GPX" ), layer="waypoints", verbose = FALSE)
```
...
```{r, echo=FALSE}
# Read competition
compe <- read.csv(here::here("/data/competence", "competence_indices.csv"), header=TRUE, sep=',')
...
for (i in variables){ 
  ...
  file_out <- file(here::here("out/anovas_competition", paste0("aov_", i, ".txt")), "w")
  sink(file_out)
  cat("MODEL \n")
  print(out$mymodel) 
  cat("\n")
...
for (i in variables){ 
  ...
  file_out <- file(here::here("/out/anovas_topo", paste0("aov_", i, ".txt")), "w")
  sink(file_out)

...
```{r}
xxx <- function(df, mivariable, mygroup){ 
  require('dplyr')
  require('multcomp')
  require('broom') 
  # mivariable: variabe of interest
  # mygroup: factor variable
  
  # Model formulation
  myformula <- as.formula(paste0(mivariable, " ~ ", mygroup))
  ...
  # Multiple comparison 
  tuk <- glht(mymodel, linfct = mcp(mygroup = "Tukey"))
...
ala <- xxx(df=topo, mivariable='slope', mygroup='site' )

...
resilience_metrics <- c("0_rt" = "Resistance", "1_rc" = "Recovery", "2_rs" = "Resilience")
...
m %>% 
  ggplot(aes(x = disturb_year, y = M.Huber, 
                   color = site, group = site, 
                   fill = site,
                   ymin = lower.ci, ymax = upper.ci)) + 
  geom_bar(stat = "identity", position="dodge", width=w) +
  geom_errorbar(width=.2, position = pos) + 
  facet_grid(var_sorted~vari, labeller = labeller(
    var_sorted = resilience_metrics, 
    vari = variables), 
  ...
  ylab("Response value")+
  xlab('Drought event') + 
  geom_text(aes(label=Letter, y = (M.Huber + .05*lower.ci)), position=pos, vjust=-2) + 
  scale_color_manual(values=c('gray', "#104E8B", 'gray', "#1E90FF", "#104E8B")) +
  scale_fill_manual(values=c('gray', "#104E8B", 'gray', "#1E90FF", "#104E8B"))

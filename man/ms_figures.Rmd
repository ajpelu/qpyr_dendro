...
**Figure 2.** EVI standardized anomaly ($EVI~sa$) during the period 2000-2016 for northern and southern populations. Error bars show standard error. 
...
themeR <- list(
  ...
  theme_bw(),
  facet_wrap(~vari, labeller = labeller(vari = variables), scales="free_y"),
  xlab(''), 
  theme(
    panel.grid.minor = element_blank(),
    strip.background = element_rect(
    ...
    plot.title = element_text(hjust = -0.05, size = 16, margin = margin(t=-5, b=1))),
  scale_shape_manual(
    values=c("N" = 19, "S" = 21, "CA-High"=22, "CA-Low" = 15, "SJ" = 17)), 
  scale_color_manual(
    values=c("N" = 'black',"S" = "blue", "CA-High" = 'blue', "CA-Low" = 'blue', "SJ" = 'black')),
  scale_fill_manual(
    values=c("N" = 'black', "S" = "white", "CA-High" = 'white', "CA-Low" = 'blue', "SJ" = 'black')),
  scale_linetype_manual(values=c("solid","solid","solid","solid", "solid")),
  scale_size_manual(
    values=c("N" = 2, "S" = 2, "CA-High"=2.5, "CA-Low" = 2.5, "SJ" = 2.5)),
...
cronos_bai <- cro_sites %>% filter(n_trees >5) %>% 
  ...
  geom_smooth(data = subset(cro_sites, year > 1975 & site == 'CA_High'), 
...
**Figure 4.** Basal Area Increment (BAI) chronologies of *Q. pyrenaica* for northern population (SJ; *green*) and southern ones: low-elevation (CA_Low; *pink*) and high-elevation (CA_High, *purple*) sites. Shading areas coorespond to standard error of the mean. Number of series are displayed in the upper plot. We only show chronologies with # trees > 5. 
...
label_site <- c("0_sj" = "Northern (SJ)",
                "1_caH" = "Southern (Ca-High)",
                "2_caL" = "Southern Low (Ca-Low)")
...
c <- gc_todos %>% 
  ...
  facet_wrap(~site_sorted, nrow = 2,
             labeller=as_labeller(c("1_caH" = "Southern (CA-High)", "2_caL" = "Southern(CA-Low)"))) + 
...
##### 
**Appendix S1** Acumulated monthly precipitation during the hydrological year 2004-2005 (blue line) and 2011-2012 (red line). The boxplot representing the average from 1940-2015 period. Data from meteorological station Granada, Base Aérea (National Spanish Meteorological Services, AEMET). 
...
**Appendix S2**. Drought severity in the Sierra Nevada for the 1950-2016 period based on the Standardised Precipitation-Evapotranspiration Index (SPEI). We used data from Global SPEI database. We obtanied the SPEI data for a 12 month scale and for all 0.5º grid cells covering Sierra Nevada.  
...
**Appendix S3.** Temporal evolution of cumulative precipitation (hydrological year) during the period 1950-2017 for 28 meteorological stations (from National Spanish Meteorological Services, AEMET) distributed around the Sierra Nevada area. Points represent mean and errorbars standard error. *Black* line indicates mean for all period. *Red* lines represent -1 (*dotted* line) and -2 (*dashed* line) standard deviation. *Blue* lines represent +1 (*dotted* line) and +2 (*dashed* line) standard deviation. We indicate all years with average values below -1SD. 
...
**Appendix S4.** Tabla de drought severas. 

##### 
**Appendix S5.** Percentage of pixels showing browning, greenning or no-changes during the 2005 and 2012 droguht events according to EVI standardized anomalies.  
...
**Appendix S6.** Reilience vs. droughts 

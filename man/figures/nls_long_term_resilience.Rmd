...
TE Quedaste por aquí. No acabaste de encontrar un NLS mejor que la LM 
...
m1 <- nls(fe1, data = d, start = list(a=2.755, b=.00014, c=.85), 
          control = list(maxiter = 500), trace = TRUE)
summary(m1)
abline(lm(rs~sev, data=d), col="blue")





summary(lm(rs~sev, data=d))


grid.start <- expand.grid(list(
  a = seq(0.001, 0.03, by=0.001),
  b = seq(1.8, 2.2, by=0.01),
  c= c(0.8)))

m2 <- nls2(rs~pF(sev, a,b,c), data = d, 
          start = grid.start,
          algorithm = "brute-force")



preview(fe1, data = d, start = list(a= 0.5, b = 0.23, c=0.8))


e1 <- nls(fe1, data = d, start = list(a= 0.5, b = 0.18, c=0.8),

...
limitesX <- c(1925, 2020)

eje_x <- scale_x_continuous(breaks = seq(1925, 2020, by=10), limits=c(1925,2020))
```
...
iv_plot <- ivv_mean_year %>% 
  ...
  theme(panel.grid = element_blank(), 
        ...
        axis.ticks.x = element_blank(), 
        axis.text.x = element_blank()) + 
  scale_x_date(limits = c(as.Date("1920-01-01"), as.Date("2020-12-31"))) 
...
p_spei <- df %>% 
  ...
  scale_x_date(limits = c(as.Date("1901-01-01"), as.Date("2016-12-31")), date_labels = "%Y", date_breaks ="5 years")
```  
...
m <- p_spei + iv_plot + sa + a+b+c+plot_layout(ncol = 1)
```



```{r}
saveImage(m, path="./man/figures/svg/", dpi=300, w=25, h=15, fig= "plot_metrics", u = "cm") 
```

...
lms <- data.frame(year = as.numeric(c(2018, 2018, 2018)),
                     ...
                     y = as.numeric(c(as.numeric(predict(mCaL)[40]), 
...
cronos_bai <- cro_sites %>% filter(nseries > 5) %>% 
  ...
  geom_text_repel(data = lms, 
                  aes(x=year, y = y, colour = site, label = label),
                  nudge_x = Inf, 
...
**Figure 4.** Basal Area Increment (BAI) chronologies of *Q. pyrenaica* for northern population (SJ; *orange*) and southern ones: low-elevation (CA-Low; *blue*) and high-elevation (CA-High, *black*) sites. Shading areas correspond to standard error of the mean. Number of series are displayed in the upper plot. We only show years replicated with # series > 5. Linear trends since 1975 are shown all sites (numbers indicate $r^2$ values). 

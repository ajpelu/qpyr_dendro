...
plotGC <- function(df, y, yse, x, my_ylab, micolor, xrange, ...){ 
  ...
  p <- ggplot(df, aes_string(x = x, y = y)) +
    ...
    scale_x_continuous(limits = c(min(xrange), max(xrange)), 
                       breaks = xrange, 
                       sec.axis = dup_axis())
  p
}                 
...
gc_todos %>% dplyr::filter(site == 'gc_caH') %>% 
  plotGC(y = 'gc_md', yse = 'gc_md_se', x = 'year', 
       my_ylab = '% GC', micolor = "#838B8B", xrange=xrange)

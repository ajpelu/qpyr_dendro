...
combinado2 <- ggplot(evi_profile_dat, aes(cd, y=mean)) + 
  ...
  geom_segment(data = dy, aes(y = mean, x= iD, xend =eD, yend = mean, colour = d_severity), 
               size = 6, alpha = 0.7) +
  geom_text(data = dy, aes(label = year, x = (iD + floor(eD-iD)/2)), size = 3) +
  scale_colour_gradient(low = "orange", high = "brown", 
                        guide = guide_colourbar(direction = "horizontal")) + 
  labs(colour = "Drought \n Severity") +
  geom_errorbar(aes(ymin = mean - 10*se, ymax= mean + 10*se), width=4, colour=micolor, size=.6) + 
  geom_line(colour=micolor, size=.6) + 
  geom_point(size=3,colour=micolor) +
  geom_point(size=1.5, colour='white') + 
  ylab('EVI') + xlab('') 
  

combinado2 <- combinado2 + 
...
```{r}
# SAVE as eps 
saveImage(combinado2, path="./man/figures/svg/", dpi=300, w=20, h=14, fig= "fig_s7", u = "cm") 
```

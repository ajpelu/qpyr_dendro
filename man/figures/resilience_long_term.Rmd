...
plotrelsite <- function(df, v, x, label, site, coordX_eq, coordY_eq, titulo, ...){
  require(ggplot2)
  require(ggrepel)
  require(purrr)
  ...
  ggplot(df, 
    ...
    geom_smooth(method='lm', se=FALSE, aes(group=1), colour='black', linetype = "dashed", size = 1) +
    geom_smooth(method='lm', se=FALSE) + 
    geom_point(size=1.5, pch = 21) + 
    theme_bw() + 
    geom_text_repel(nudge_x = 0.5, size = 3, show.legend = FALSE) + 
    # annotate("text", x = coordX_eq, y = coordY_eq, 
    #          label=lm_eqn(lm(formulita, df)), parse=TRUE, 
    #          size = 3, fontface = "bold", hjust="left") +
    ggtitle(titulo) + 
    theme(plot.title = element_text(size = 11, face = "bold.italic"),
          ...
          axis.text.x = element_text(size=9),
          axis.title = element_text(size = 10)) +
    geom_text_repel(data = eqns, aes(x = -Inf, y = Inf, label = equation), 
                    direction = "y", 
...
sevRs_site <- plotrelsite(sdfsite, v = "rs", x = 'd_severity', label = 'disturb_year', site = "site",
                 coordX_eq = 6, coordY_eq = 0.5, titulo = "Rs") + 
  xlab('') + ylab('') + theme(plot.margin = margin(r=0, l=-0.7, t=0, b=0, "pt")) 
```
...
ggsave(filename = here::here("./man/figures/svg/fig_6.jpg"), 
       dpi = 300,  plot=res_long_term_sites, width=10, height=8)
